...
logd_50 <- read.csv("50-2020-07-26-08-40-46-YAs10RS8.exec/samples/logDensity.csv")
logd_1 <- read.csv("1-chain_2020-07-26-08-40-57-RtGpUME2.exec/samples/logDensity.csv")

combined <- bind_rows(f0_50, f0_1, logd_50, logd_1, .id = "id") %>% 
  filter(sample >= 2500) %>%
  mutate(
    statistic = ifelse(id == 1 | id == 2, "X_i", "V(X)"),
    sampler = ifelse(id == 1 | id == 3, "Non reversible PT (50 chains)", "Exploration kernel alone (1 chain)")
  )
